...
void forward_s2c(int s_fd, int c_fd)
{
    ...
    int pr = PR_NONE;
    size_t content_len = get_response_priority(&(rsp_header.head), &pr);
    printf("pr=%d\n", pr);
        ...
        case PR_CONTENT_LEN:  /* 嵌套太多,不合适 */
            {
                if(content_len > 0)
                {
                    ...
                    if(( n = readn(s_fd, buf_body, content_len)) == content_len)
                    {
                        ...
                        {
                            rewrite_content_length(&(rsp_header.head), strlen((char *)new_body));
                            forward_http_header(c_fd, &rsp_header, RESPONSE);
                            forward_http_body(c_fd, (char *)new_body, strlen((char *)new_body));
                            SAFE_FREE(buf_body);
                            SAFE_FREE(new_body);
                        }
                    }
