...
int main(int argc, char **argv)
{
    ...
    list_for_each(pos, head)
    {
        ...
        //printf("%p %p", temp, pos);
        printf("address=%p index=%d startoffset=%d len_substr=%d substr=%s\n", temp, temp->index, temp->startoffset, temp->len_substr, temp->substr);
    }
    pos = head->next;
    while(pos != head)
    {
    ...
    SAFE_FREE(pat);
    printf("SAFE_FREE pat =%p\n", pat);
    SAFE_FREE(sub);
    printf("SAFE_FREE sub =%p\n", sub);
    pcre2_match_data_free(match_data);
    pcre2_code_free(re);
    return 0;
}
