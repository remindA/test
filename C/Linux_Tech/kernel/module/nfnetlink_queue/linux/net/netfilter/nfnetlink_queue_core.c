...
#include <net/netfilter/nf_conntrack.h>   /* NYB */
#include <net/netfilter/nf_nat_helper.h>  /* NYB */
#include <linux/atomic.h>
#include <net/route.h>  /* NYB */
...
nfqnl_recv_verdict(struct sock *ctnl, struct sk_buff *skb,
		   ...
		   const struct nlattr * const nfqa[])
{
    printk(KERN_EMERG"=== nfqnl_recv_verdict() start\n");
	struct nfgenmsg *nfmsg = nlmsg_data(nlh);
	...
	if (nfqa[NFQA_PAYLOAD]) {
        ...
        printk(KERN_EMERG"nfqnl_recv_verdict: payload_len = %d\n", payload_len);
		int diff = payload_len - entry->skb->len;

		if (nfqnl_mangle(nla_data(nfqa[NFQA_PAYLOAD]),
				 payload_len, entry, diff) < 0) {
            printk(KERN_EMERG"verdict become DROP\n");
			verdict = NF_DROP;
        }
        /* NYB add: 直接从entry->skb中解析出ct,而不是从nfa中解析出 */
        if(diff) {
            ct = nf_ct_get(entry->skb, &ctinfo);
        }
		if(ct) {
            printk(KERN_EMERG"nfqnl_recv_verdict: will do ct_seq_adjust\n");
			nfqnl_ct_seq_adjust(entry->skb, ct, ctinfo, diff);
        }
        
        /* NYB add this */
        if(rt && payload_len > dst_mtu(&rt->dst)) {
            printk(KERN_EMERG"payload_len = %d, dst_mtu = %d\n", payload_len, dst_mtu(&rt->dst));
            entry->skb->local_df = 1;
        }
        printk(KERN_EMERG"nfqnl_recv_verdict: entry->skb->local_df = %d\n", entry->skb->local_df);
#if 0
        /* NYB comment: */
		if (ct)
	...
	nf_reinject(entry, verdict);
    printk(KERN_EMERG"=== nfqnl_recv_verdict() end\n");
	return 0;
}
